...
func readConfigV2(path string) (configV2, error) {
	p, err := ioutil.ReadFile(path)
	if err != nil {
		log.Fatalln(err)
	}
	c := newDefaultConfigV2()
	if err := toml.Unmarshal(p, &c); err != nil {
		log.Fatalln(err)
	}

	if c.Version != configVersion {
		return configV2{}, fmt.Errorf("unknown file version %v; please upgrade to %v", c.Version, configVersion)
	}
